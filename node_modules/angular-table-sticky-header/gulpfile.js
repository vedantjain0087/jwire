...
var
    config = {
        ...
        'moduleConfig': {
            ...
            'exports': 'angular.module("ngTableStickyHeader")',
            'type': 'umd'
        }
    };
...
gulp.task('build', function (cb) {
    return gulpSequence(['module-licenses', 'styles-sass', 'scripts-js', 'docs'], cb);
});
...
/**
 * Builds the SCSS/SASS
 */
gulp.task('styles-sass', function () {
    return gulp.src(config.sass)
        .pipe(gulpPlumber())
        // execute sass on the source files
        .pipe(gulpSass({'includePaths': config.sassImportPaths, 'outputStyle': production ? 'compressed' : 'expanded'}))
        // merge to one file
        .pipe(gulpConcat(production ? config.moduleConfig.name + '.min.css' : config.moduleConfig.name + '.css'))
        // write css
        .pipe(gulp.dest(config.dist));
});
